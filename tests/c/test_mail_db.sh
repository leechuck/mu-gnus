...
if $MAIL_DB add "msg001" --from "alice@example.com" --subject "Hello World" --db "$TEST_DB" > /dev/null 2>&1; then
    echo -e "${GREEN}PASS${NC}"
else
    echo -e "${RED}FAIL${NC}"
    exit 1
...
if $MAIL_DB add "msg002" --from "bob@example.com" --subject "Important Question" --needs-reply --classification "work" --db "$TEST_DB" > /dev/null 2>&1; then
    echo -e "${GREEN}PASS${NC}"
else
    echo -e "${RED}FAIL${NC}"
    exit 1
...
if $MAIL_DB add "msg003" --from "charlie@example.com" --subject "Old Message" --date 1609459200 --db "$TEST_DB" > /dev/null 2>&1; then
    echo -e "${GREEN}PASS${NC}"
else
    echo -e "${RED}FAIL${NC}"
    exit 1
...
RESULT=$($MAIL_DB query --db "$TEST_DB" 2>/dev/null)
...
RESULT=$($MAIL_DB query --needs-reply --db "$TEST_DB" 2>/dev/null)
...
RESULT=$($MAIL_DB query --from "alice@example.com" --db "$TEST_DB" 2>/dev/null)
...
RESULT=$($MAIL_DB query --classification "work" --db "$TEST_DB" 2>/dev/null)
if echo "$RESULT" | grep -q "msg002" && ! echo "$RESULT" | grep -q "msg001"; then
    echo -e "${GREEN}PASS${NC}"
else
    echo -e "${RED}FAIL${NC}"
...
if $MAIL_DB update "msg002" --replied 1 --db "$TEST_DB" > /dev/null 2>&1; then
    echo -e "${GREEN}PASS${NC}"
else
    echo -e "${RED}FAIL${NC}"
    exit 1
...
RESULT=$($MAIL_DB query --not-replied --db "$TEST_DB" 2>/dev/null)
if ! echo "$RESULT" | grep -q "msg002" && echo "$RESULT" | grep -q "msg001"; then
    echo -e "${GREEN}PASS${NC}"
else
    echo -e "${RED}FAIL${NC}"
...
if $MAIL_DB update "msg001" --classification "personal" --db "$TEST_DB" > /dev/null 2>&1; then
    echo -e "${GREEN}PASS${NC}"
else
    echo -e "${RED}FAIL${NC}"
    exit 1
...
RESULT=$($MAIL_DB query --classification "personal" --db "$TEST_DB" 2>/dev/null)
...
if $MAIL_DB update "msg001" --needs-reply 1 --db "$TEST_DB" > /dev/null 2>&1; then
    echo -e "${GREEN}PASS${NC}"
else
    echo -e "${RED}FAIL${NC}"
    exit 1
...
if $MAIL_DB add "msg004" --from "test@example.com" --subject "Quote\" and \\ backslash" --db "$TEST_DB" > /dev/null 2>&1; then
    echo -e "${GREEN}PASS${NC}"
else
    echo -e "${RED}FAIL${NC}"
    exit 1
...
if ! $MAIL_DB update "nonexistent" --replied 1 --db "$TEST_DB" > /dev/null 2>&1; then
    echo -e "${GREEN}PASS${NC}"
else
    echo -e "${RED}FAIL${NC}"
    exit 1
...
if ! $MAIL_DB add "msg001" --from "duplicate@example.com" --subject "Duplicate" --db "$TEST_DB" > /dev/null 2>&1; then
    echo -e "${GREEN}PASS${NC}"
else
    echo -e "${RED}FAIL${NC}"
    exit 1
...
$MAIL_DB add "msg005" --from "alice@example.com" --subject "Another message" --needs-reply --db "$TEST_DB" > /dev/null 2>&1
RESULT=$($MAIL_DB query --needs-reply --from "alice@example.com" --db "$TEST_DB" 2>/dev/null)
